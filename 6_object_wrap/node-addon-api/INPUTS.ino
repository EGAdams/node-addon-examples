...
Inputs::Inputs( Player *player1, Player *player2, PinInterface *pinInterface, GameState *gameState ) :
...
void Inputs::readReset() {
    ...
    if ( _pinInterface->digitalRead( RESET ) == LOW  ) {
        // Serial.println( "Reset press detected _pinInterface->digitalRead( RESET ) == LOW infinite loop starts here..." );
...
void Inputs::readUndoButton() {
    ...
    if ( _pinInterface->digitalRead( UNDO ) == LOW  ) {
        Serial.println( "entering _pinInterface->digitalRead( UNDO ) == LOW infinite loop...  waiting for it to go HIGH..." );
...
void Inputs::readPlayerButtons() {
    ...
    if ( anlgPlyrBtnVal <= 1000 ) { // if one of the player buttons is pressed...
        ...
        while ( _digiFunctions->analogRead( PLAYER_BUTTONS ) <= 1000  ) {
            ...
            // _digiFunctions->gameDelay( 1000 );                          // Be careful with the infinite loop!!!
        }
